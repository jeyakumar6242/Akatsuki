...
public class BreezeConnect extends TradingUtility {
	...
	public Dictionary getDataFromStockTokenValue(String inputStockToken) throws Exception {
		...
		if (exchangeCodeName.equals("")) {
			...
			if (stockData == null) {
				this.errorException(
						String.format(config.exceptionMessage.get(Config.ExceptionEnum.STOCK_NOT_EXIST_EXCEPTION),
								"BSE", inputStockToken));
			}
			...
			if (stockData == null) {
				...
				if (stockData == null) {
					this.errorException(
							String.format(config.exceptionMessage.get(Config.ExceptionEnum.STOCK_NOT_EXIST_EXCEPTION),
									"i.e. NSE or NFO.", inputStockToken));
				} else {
	...
	public Map<String, String> getStockTokenValue(String exchangeCode, String stockCode, String productType,
			String expiryDate, String strikePrice, String right, boolean getExchangeQuotes, boolean getMarketDepth)
			throws Exception {
				...
				if (exchangeCode.toLowerCase().equals("bse"))
					...
					if (productType.toLowerCase().equals("options")) {
						if (strikePrice.equals(""))
							this.errorException(
									config.exceptionMessage.get(Config.ExceptionEnum.STRIKE_PRICE_EXCEPTION));
						else
	...
	public void generateSession(String secretKey, String sessionToken) throws Exception {
		try {
			...
			httpGet.setURI(URI.create(
					config.urls.get(Config.UrlEnum.API_URL) + config.endPoints.get(Config.EndPointEnum.CUST_DETAILS)));
			...
			if (responseJson.has("Success") && !responseJson.isNull("Success")
					&& !responseJson.getString("Success").isEmpty() && !responseJson.getString("Success").isBlank()) {
	...
	public void connect(boolean isOrder, boolean isOHLC) {
		...
		if (isOrder && (this.socketOrder == null)) {
			IO.Options options = IO.Options.builder().setTransports(new String[] { WebSocket.NAME }).setAuth(authDict)
					.build();
	...
	public JSONObject subscribeFeeds(String stockToken) throws Exception {
		try {
			...
			return this.socketConnectionResponse(
					String.format(config.responseMessage.get(Config.ResponseEnum.STOCK_SUBSCRIBE_MESSAGE), stockToken));
		} catch (Exception e) {
			this.errorException(e.toString());
		}
		return null;
	...
	public JSONObject subscribeFeeds(String stockToken, String interval) throws Exception {
		try {
			if (!Objects.equals(interval, "")) {
				if ((!(Arrays.asList(config.typeLists.get(Config.ListEnum.INTERVAL_TYPES_STREAM_OHLC)))
	...
	public JSONObject subscribeFeeds(boolean getOrderNotification) throws Exception {
		try {
			if (Objects.isNull(getOrderNotification) || !getOrderNotification)
				return this.socketConnectionResponse(
						String.format(config.responseMessage.get(Config.ResponseEnum.ORDER_FLAG_NOT_SET), "true"));
			else if (orderNotificationSubscribed)
				return this.socketConnectionResponse(
						config.responseMessage.get(Config.ResponseEnum.ORDER_NOTIFICATION_SUBSCRIBED));
			else {
	...
	public JSONObject subscribeFeeds(String exchangeCode, String stockCode, String productType, String expiryDate,
		...
		try {
			Map<String, String> tokenObject = this.getStockTokenValue(exchangeCode, stockCode, productType, expiryDate,
					strikePrice, right, getExchangeQuotes, getMarketDepth);
			...
			return this.socketConnectionResponse(
					String.format(config.responseMessage.get(Config.ResponseEnum.STOCK_SUBSCRIBE_MESSAGE), stockCode));
		} catch (Exception e) {
			this.errorException(e.toString());
		}
		return null;
	...
	public JSONObject subscribeFeeds(String exchangeCode, String stockCode, String productType, String expiryDate,
			String strikePrice, String right, boolean getExchangeQuotes, boolean getMarketDepth, String interval)
			throws Exception {
		try {
			if (!Objects.equals(interval, "")) {
				if ((!(Arrays.asList(config.typeLists.get(Config.ListEnum.INTERVAL_TYPES_STREAM_OHLC)))
	...
	public JSONObject unsubscribeFeeds(String stockToken) throws Exception {
		try {
			...
			return this.socketConnectionResponse(String
					.format(config.responseMessage.get(Config.ResponseEnum.STOCK_UNSUBSCRIBE_MESSAGE), stockToken));
		} catch (Exception e) {
			this.errorException(e.toString());
		}
		return null;
	...
	public JSONObject unsubscribeFeeds(boolean getOrderNotification) throws Exception {
		try {
			if (Objects.isNull(getOrderNotification) || getOrderNotification)
				return this.socketConnectionResponse(
						String.format(config.responseMessage.get(Config.ResponseEnum.ORDER_FLAG_NOT_SET), "false"));
			else if (!orderNotificationSubscribed)
				return this.socketConnectionResponse(
						config.responseMessage.get(Config.ResponseEnum.ORDER_REFRESH_DISCONNECTED));
			else {
	...
	public JSONObject unsubscribeFeeds(String exchangeCode, String stockCode, String productType, String expiryDate,
		...
		try {
			...
			return this.socketConnectionResponse(String
					.format(config.responseMessage.get(Config.ResponseEnum.STOCK_UNSUBSCRIBE_MESSAGE), stockCode));
		} catch (Exception e) {
			this.errorException(e.toString());
		}
		return null;
	...
	private Object parseData(Object data) throws Exception {
		...
		JSONArray jArray = (JSONArray) data;
		if (jArray != null && jArray.get(0).getClass().getSimpleName().equals("String")
				&& jArray.get(0).toString().indexOf('!') < 0) {
			...
			if (jArray.get(11).equals("4") || jArray.get(11).equals("5")) {
				...
				orderObject.put("orderFlow", this.tuxToUserValue.get("orderFlow")
						.getOrDefault(jArray.get(15).toString().toUpperCase(), jArray.get(15).toString())); // Order
																											// Flow
				orderObject.put("limitMarketFlag", this.tuxToUserValue.get("limitMarketFlag")
						.getOrDefault(jArray.get(16).toString().toUpperCase(), jArray.get(16).toString())); // Limit
																											// Market
																											// Flag
				orderObject.put("orderType", this.tuxToUserValue.get("orderType")
						.getOrDefault(jArray.get(17).toString().toUpperCase(), jArray.get(17).toString())); // OrderType
				orderObject.put("orderLimitRate", jArray.get(18)); // Limit Rate
				orderObject.put("productType", this.tuxToUserValue.get("productType")
						.getOrDefault(jArray.get(19).toString().toUpperCase(), jArray.get(19).toString())); // Product
																											// Type
				orderObject.put("orderStatus", this.tuxToUserValue.get("limitMarketFlag")
						.getOrDefault(jArray.get(20).toString().toUpperCase(), jArray.get(20).toString())); // Order
																											// Status
				...
				orderObject.put("productType", this.tuxToUserValue.get("productType")
						.getOrDefault(jArray.get(15).toString().toUpperCase(), jArray.get(15).toString())); // Product
																											// Type
				orderObject.put("optionType", this.tuxToUserValue.get("optionType")
						.getOrDefault(jArray.get(16).toString().toUpperCase(), jArray.get(16).toString())); // Option
																											// Type
				...
				orderObject.put("orderFlow", this.tuxToUserValue.get("orderFlow")
						.getOrDefault(jArray.get(21).toString().toUpperCase(), jArray.get(21).toString())); // Order
																											// Flow
				orderObject.put("limitMarketFlag", this.tuxToUserValue.get("limitMarketFlag")
						.getOrDefault(jArray.get(22).toString().toUpperCase(), jArray.get(22).toString())); // Limit
																											// Market
																											// Flag
				orderObject.put("orderType", this.tuxToUserValue.get("orderType")
						.getOrDefault(jArray.get(23).toString().toUpperCase(), jArray.get(23).toString())); // Order
																											// Type
				orderObject.put("limitRate", jArray.get(24)); // Limit Rate
				orderObject.put("orderStatus", this.tuxToUserValue.get("orderStatus")
						.getOrDefault(jArray.get(25).toString().toUpperCase(), jArray.get(25).toString())); // Order
																											// Status
	...
	public Object parseOHLCData(Object data) throws JSONException {
		...
		} else if (Objects.equals(arrayData[0], "NFO") || Objects.equals(arrayData[0], "NDX")
				|| Objects.equals(arrayData[0], "MCX")) {
